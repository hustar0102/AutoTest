...
public class TestCase {
	...
	/**
	 * 打log用的对象,this表示具体的子类。 print log
	 */
	protected Log log = LogFactory.getLog(this.getClass());
	/**
	 * 决定这个TestCase是用什么浏览器的driver来执行。
	 * 由于设置了BeforeMethod标签，这个方法将由TestNG在每个TestMethod被执行前调用。
	 * 他将接收一个从TestNG的xml文件传入的参数表示浏览器种类。 告诉manager我要新建的driver的类型。 runs by testNG,
	 * it will be run before every test method to decide the driver type
	 * 
	 * @Parameters browser：从testng的xml文件传入的浏览器名称。 @optional("chrome")的意思是：
	 *             如果在testng.xml文件中没有找到名为"browser"的参数，测试方法将接受在@Optional注解中指定的默认值
	 *             ："chrome"
	 */
	...
	/**
	 * 在一个测试方法结束时关闭driver runs by testNG, it will be run after every test method
	 * to close the driver
	 */
...
//		 DriverManager.quitDriver();
	...
	/**
	 * 打印类名。建议一个CASE只放一个方法。 Print Class Name
	 */
	...
	/**
	 * 再次打印类名 Print Class Name again and separator
	 */
	...
	public static class DriverManager {
		/**
		 * 每个DriverManager只管理一个driver，所以他是static的，但是我引入ThreadLocal来处理多线程 shares
		 * the same web driver and use thread local to handle the multi-thread
		 */
		...
		/**
		 * 如果当前进程没有绑定driver，创建一个然后绑定上，如果已经有了就直接返回 create a driver for this
		 * thread if not exist. or return it directly
		 */
